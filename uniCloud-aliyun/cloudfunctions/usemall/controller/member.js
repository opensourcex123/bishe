...
module.exports = class MemberController extends Controller {
	...
	async register() {
		...
		if (res.code == 0) {
			...
			response.datas.member = await this.memberLogin(res, member, 0);
		} else {
			response.datas = res;
	...
	async loginByWeixin() {
		...
		const {res} = this.ctx.data;
		
		try {
			this.ctx.data.role = ['member'];

			let member = {
				member_name: res.nickname || '用户',
				member_nickname: res.nickname || '用户',
				member_headimg: res.avatar || '',
				member_weixin_headimg: res.avatar || '',
			}
	...
	// 写入会员登录数据
	async memberLogin(res, member, app_login_cnt) {
		let obj_id = res._id;
		...
		this.db.collection('usemall-member').doc(obj_id).update({
			...
			app_login_cnt: this.db.command.inc(app_login_cnt),
			version: this.db.command.inc(1),
			last_modify_uid: res._id,
			last_modify_time: new Date().getTime()
		}).then(res1 => {
			if (res1 && res1.updated <= 0) {
		...
		const memberRes = await this.db.collection('usemall-member').doc(res._id)
			.field({
				member_session_key: false,
				member_password: false
			}).get();
