...
module.exports = class OrderController extends Controller {
	...
	async create() {
		...
		let order = {
			...
			state: '待接单',
			is_delete: 0,
			create_uid: create_uid || uid,
			create_time: new Date().getTime(),
			imgs: imgs,
			start_time: start_time,
			end_time: end_time,
			time: time,
			introduce_engineer_id: introduce_engineer_id,
	...
	async detail() {
		...
		const order_log = this.db.collection('usemall-order-log').where({
			order_id: order_id
		}).get();
		const datas = await Promise.all([order, order_detail, order_log]);

		response.order = datas[0] && datas[0].data && datas[0].data[0];
		response.order_detail = datas[1] && datas[1].data;
		response.order_log = datas[2] && datas[2].data;

		// 返回待付款订单剩余支付时间
		// if (response.order.state == '待接单') {
		// 	// 默认1小时
		...
		switch (pay_way) {
				...
				switch (pay_trade_type) {
					...
					default:
						...
						wxpayOrderInfo = await uniWxpayIns.getOrderInfo({
							openid: openid,
							body: '家庭维修服务系统',
							outTradeNo: new Date().getTime() + (Math.round(Math.random() * 1000000) +
								'').padStart(6, '0'),
							totalFee: money * 100,
							notifyUrl: uidObj.config['mp-weixin']['notify-url']
						});
			...
			case '支付宝':
				...
				let alipayOrderInfo = await uniAlipayIns.getOrderInfo({
					...
					spbillCreateIp: this.ctx.CLIENTIP,
					tradeType: pay_trade_type
				});
	...
	async pay() {
		...
		switch (pay_way) {
			...
			case '支付宝':
				const fs = require('fs');
				const path = require('path');
				const alipayConfig = uidObj.config['app']['alipay'];
	...
	async paystate() {
		...
		// 查询订单支付状态
		const payRes = await this.db.collection('usemall-order-pay').where({
			order_id
		}).get();
	...
	async checkUnderwayData() {
		...
		const res = await this.db.collection("usemall-order").where(dbcmd.or({
			state: dbcmd.in(['待接单', '已接单', '已上门', "待推送报价", "已完成报价", "待结算", "上传维修前照片", "上传维修后照片",
				"选择质保卡", "拒绝报价"
			]),
			engineer_id: uid
		}, {
	...
	async checkTodayData() {
		...
		where({
				end_time: dbcmd.gt(todayStart).lt(todayEnd),
				engineer_id: uid,
				state: dbcmd.in(['待接单', '已接单', '已上门', "待推送报价", "已完成报价", "待结算", "上传维修前照片", "上传维修后照片",
					"选择质保卡", "拒绝报价"
				])
	...
	async checkReassignmentData() {
		...
		const res = await this.db.collection("usemall-order").where({
			state: '待改派',
			engineer_id: uid
		}).orderBy("create_time", "desc").skip((page - 1) * pageSize).limit(pageSize).get()
	...
	async checkTomorrowData() {
		...
		const tomorrowEnd = dayjs().add(1, "day").endOf('day').valueOf()

		const res = await this.db.collection("usemall-order").
		where({
				end_time: dbcmd.gt(tomorrowStart).lt(tomorrowEnd),
				engineer_id: uid,
				state: dbcmd.in(['待接单', '已接单']),
			})
			.orderBy("create_time", "desc").skip((page - 1) * pageSize).limit(pageSize).get()
		return {
			code: 200,
	...
	async getOrderAddrDetails() {
		const {
			addr_id,
		} = this.ctx.data

		const res = await this.db.collection("usemall-member-address").doc(addr_id).get()
	...
	async getOrderGoodPriceList() {
		const {
			order_id
		} = this.ctx.data

		const res = await this.db.collection("usemall-order-detail").where({
			order_id: order_id
		}).get()
	...
	async getOrderListByStateAndTime() {
		const {
			pageNo,
			pageSize,
			states,
		...
		const res = await this.db.collection("usemall-order").
		where({
				create_time: dbcmd.gt(start_time).lt(end_time),
				engineer_id: uid,
				state: dbcmd.in(states),
			})
			.orderBy("create_time", "desc").skip((pageNo - 1) * pageSize).limit(pageSize).get()
		return {
			code: 200,
			data: res
		}
	...
	async getEngineerOrderDetails() {
		const {
			order_id
		} = this.ctx.data

		const res1 = await this.db.collection("usemall-order").where({
			order_id: order_id
		}).get()

		const res2 = await this.db.collection("income").where({
			order_id: order_id
		}).get()

		const res3 = await this.db.collection("warranty-card").
		where({
			_id: dbcmd.in(res1.data[0].warranty_card_id)
		}).
		get()
	...
	async getFilterEngineer() {
		...
		const uidRes = await this.db.collection('usemall-order').where({
			order_id: order_id
		}).get()
		const cidRes = await this.db.collection('uni-id-device').where({
			user_id: uidRes.data[0].create_uid
		}).get()
		// 获取登录的用户的设备id
		const _cid = cidRes.data[0].push_clientid;
		let filter_1 = [];
		// 获取技能
		let skills = [];
		skills.push(name);
		const res1 = await this.db.collection('usemall-goods-category').where({
			_id: cid
		}).get()
		skills.push(res1.data[0].name);
		const res2 = await this.db.collection('usemall-goods-category').where({
			_id: res1.data[0].pid
		}).get()
		skills.push(res2.data[0].name);

		let engineers = await this.db
			.collection('engineers')
			.where({
				point: dbcmd.geoNear({
					geometry: new this.db.Geo.Point(parseFloat(longitude), parseFloat(latitude))
				}),
				status: 1,
				audit_status: 1,
				skill: dbcmd.in(skills)
			})
			.get();
		const engineerList = engineers.data;
		// 过滤接单数量
		let start = new Date(uidRes.data[0].start_time).setHours(0, 0, 0, 0);
		let end = new Date(uidRes.data[0].end_time).setHours(23, 59, 59, 999);
		for (var i = 0; i < engineerList.length; i++) {
			const countRes = await this.db.collection('usemall-order')
				.where({
					engineer_id: engineerList[i].user_id,
					state: dbcmd.nin(['待接单', '已取消', '待改派', '待取消']),
					start_time: dbcmd.gt(start),
					end_time: dbcmd.lt(end)
				})
				.count()
			if (countRes.total < engineerList[i].limit) {
				filter_1.push(engineerList[i]);
			}
		}
		...
		const engineerCidRes = await this.db.collection('uni-id-device').where({
		...
		return {
			...
			user_cid: _cid,
			engineer_cid: engineerCidRes.data[0].push_clientid
		}
	}

	async updateOrderImgs() {
		const {
			order_id,
			imgs
		} = this.ctx.data
	...
	async updateChargeStandard() {
		...
		const res = await this.db.collection("usemall-order-detail").where({
			...
			charge_standard: dbcmd.push(charge_standard)
		})
		return {
			code: 200
		}
	...
	async checkPendingCancelStateOrder() {
		const {
			page,
			pageSize
		} = this.ctx.data
	...
	async updateActualPaid() {
		...
		await this.db.collection('usemall-order').where({
			...
			order_actural_paid: dbcmd.inc(price * 100)
		})
		return {
			code: 200
		}
	...
	async getVersion() {
		...
		const res = await this.db.collection('version').where({
			_id: '639ab4c253a7f300010192cd'
		}).get()
		
		if (res.data[0].version !== current_version) {
			return {
				update: true,
				url: res.data[0].url
			}
