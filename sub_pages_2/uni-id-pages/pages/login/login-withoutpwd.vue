...
export default {
	...
	computed: {
		...
		imgSrc() {
			//大快捷登录按钮图
			return url + 'login/' + this.type + '.png';
		}
	},
	async onLoad(e) {
		console.log(e);
		...
		this.$nextTick(() => {
			...
			if (['weixin', 'apple'].includes(type)) {
				this.$refs.uniFabLogin.servicesList = this.$refs.uniFabLogin.servicesList.filter(item => item.id != type && config.loginTypes.includes(item.id));
			}
		});
