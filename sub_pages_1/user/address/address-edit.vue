<template>
	<view class="content bg-drak ft-main">
		...
		<view class="row dflex border-line padding-lr">
			...
			<input class="input" type="text" v-model="addrData.consignee" placeholder="请输入联系人姓名" placeholder-class="placeholder" />
		</view>
		<view class="row dflex border-line padding-lr">
		...
		<view class="row dflex border-line padding-left">
			...
			<view class="dflex-b flex1">
				...
				<!-- #endif -->
				<!-- #ifdef MP-ALIPAY -->
				<view class="iconfont icondizhi- input_t padding-lr-sm padding-tb-16" @click="choiceAlipayMapAddr"></view>
				<!-- #endif -->
			</view>
		</view>
		<view class="row dflex border-line padding-lr">
			<text class="tit">楼号/门牌号</text>
			<input class="input" type="text" v-model="addrData.addr_detail" placeholder="例:6号楼203室" placeholder-class="placeholder" />
		</view>
		<view class="gap"></view>
...
export default {
	...
	methods: {
			...
			uni.chooseLocation({
				success: res => {
					...
					if (res && res.address) {
						...
						_this.addrData.address = [_this.addrData.province_name, _this.addrData.city_name, _this.addrData.area_name].filter(x => x).join('-');
						_this.addressName = _this.addrData.address;
		...
		async submit() {
			...
			if (this.addrDefault) {
				// 把默认为是的改成 否
				await this.$db[__name].where('create_uid == $env.uid && is_default == "是"').update({
					is_default: '否'
				});
			}
