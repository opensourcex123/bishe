		...
		<swiper :current="tabCurrentIndex" :disable-touch="true" class="swiper-box swiper-area h-full" duration="300" @change="changeTab">
			<swiper-item class="tab-content" v-for="(tabItem, tabIndex) in navList" :key="tabIndex">
				<scroll-view class="list-scroll-content h-full" scroll-y>
					<!-- 空白页 -->
					<use-empty v-if="tabItem.datas.length === 0 && tabItem.loaded" e-style="round" e-type="cart" tip="暂无发票" height="93vh"></use-empty>
						...
						<view class="coupon_box border-radius margin-top-sm bg-main">
							<view class="dflex-b">
								<view class="left pos-a h-full dflex-c dflex-flow-c">
									<view class="fs-sm">{{item.order_info}}</view>
								</view>
								<view class="right padding left_t flex1">
									<view class="dflex-b ft-dark fs-xs border-line">
										<view v-if='item.status === 0' class="">您的发票正在审核中，请耐心等待</view>
										<view v-if='item.status === 1' class="">您的发票已通过审核</view>
										<view v-if='item.status === 2' class="">您的发票未通过审核</view>
									</view>
								</view>
							</view>
						</view>
	...
	export default {
		...
		methods: {
			...
			async loadData() {
				...
				let _this = this;
				let list = [];
				let res = await db.collection('electronic-tickets')
				.where('create_uid == $env.uid')
				.get()

				list = res.result.data;
				list.forEach(item => {
					_this.$db['usemall-order'].where({order_id: item.order_id}).tofirst().then(res => {
						item.order_info = res.datas.order_info;
						if (item.status === 0) {
							_this.navList[0].datas.push(item);
						} else if (item.status === 1) {
							_this.navList[1].datas.push(item);
						} else {
							_this.navList[2].datas.push(item);
						}
					})
				});
			},
